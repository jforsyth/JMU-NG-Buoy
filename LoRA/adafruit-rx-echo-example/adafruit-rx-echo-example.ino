...
void loop()
{
  //check to see if there are any bytes from the radio
  if (rf95.available())
  {
    ...
    //read the received bytes into the buffer. Store the number of
    //bytes received into len
    if (rf95.recv(buf, &len))
    {
      ...
      //create a transmit buffer to hold your return message
      uint8_t* data = buf;
      rf95.send(data, len);
      rf95.waitPacketSent();
